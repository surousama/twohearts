...
6. **迁移期允许回退** — 在新 workflow 跑通前，旧 `docs` 继续保留
...
Code committed via Phase 3.4; run `/trellis:finish-work` to wrap up（archive task 并记录本次会话）。
...
2. **观察提交风格**，让 commit message 与项目历史一致：
   ...
   git log --oneline -5
   ```
   关注前缀（`feat:` / `fix:` / `chore:` / `docs:` 等）、语言（中文 / English）和长度风格。

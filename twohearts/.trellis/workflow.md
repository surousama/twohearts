...
1. **先判定角色，再开工** — 角色不清时，不进入实现
2. **先定边界，再定实现** — 当前目标、非目标、验收口径先写清楚
3. **Specs injected, not remembered** — 规范靠 `.trellis/spec/` 注入，不靠 AI 自己记
4. **Persist everything** — 阶段结论、实现记录、测试反馈都应落文件
5. **父 task 管阶段，子 task 管单轮验收** — 避免任务粒度失控
6. **迁移期可回退** — 在新 workflow 跑通前，旧 `docs` 保留
...
No active task. **A Direct answer** — 纯问答、阅读说明、流程解释、工具介绍、轻量查询，可直接回答。
**B Create a task** — 任何会落文档、改配置、改代码、做测试计划、做技术评估、推进真实开发阶段的工作，都应先建 task。先判定角色，再建 task，再写 prd.md。
**C Inline change** — 只有用户明确要求跳过 Trellis 流程时才允许，例如“跳过 trellis”“直接改”“先别建任务”。
[/workflow-state:no_task]
...
双心印任务必须在 `prd.md` 中写清：当前目标、当前明确不做、验收口径、与当前阶段顺序的关系。

...
* 本轮 Guard 允许从当前已存在的玩家动作状态尝试进入，不只限空闲态。
* 本轮 Guard 支持打断普通攻击 `Recovery`，并允许打断当前基础 Dodge。
* 本轮输入形态先按单击短窗口实现，不支持长按维持。
* 代码层需要预留长按扩展接口与输入释放观察口，不把本轮单击实现写死成无法扩展的形态。
* 本轮 Guard 只保证对 `05-25-minimal-hostile-attack-probe` 提供的最小敌对攻击探针生效，不扩展到所有未来来袭类型。
